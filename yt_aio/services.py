...
def _cookie_home_override(config: dict[str, Any], use_auth: bool) -> str | None:
    ...
    if config.get("cookie_fallback_home"):
        return str(Path(str(config["cookie_fallback_home"])).expanduser())
...
def build_yt_dlp_base_args(config: dict[str, Any], *, use_auth: bool = False) -> list[str]:
    ...
    if use_auth:
        if config.get("cookie_file"):
            args.extend(["--cookies", str(Path(str(config["cookie_file"])).expanduser())])
        else:
...
def build_download_command(
    ...
    *,
    ...
    download_dir = str(Path(config["default_download_path"]).expanduser())
    command = [
        "--newline",
        "--ignore-errors",
        "--no-overwrites",
