...
from ..utils.download_manager import download_many, record_user_action
from ..utils.shared import CancellationToken, DownloadTarget, VideoItem, now_string
from ..utils.video_info_extractor import list_videos, parse_quick_download_urls
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.config = resolve_runtime_config(self.raw_config)
    ...
    def reload_config_if_changed(self) -> None:
        try:
            previous_config = self.raw_config
        ...
        if json.dumps(previous_config, sort_keys=True) == json.dumps(latest_config, sort_keys=True):
            self.raw_config = latest_config
            self.config = resolve_runtime_config(latest_config)
            return
